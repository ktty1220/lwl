...
このプロパティを記述した位置の __拡張__ スタックトレース情報および呼び出し元の __拡張__ スタックトレース情報を大元まで配列で返します。意味が良く分からないかもしれませんが、スクリプトでエラーが発生した時に出るアレの元情報を取得できるわけです。
...
何が __拡張__ かというと、`lwl.__stack`で取得したスタックトレース情報には特別なプロパティが用意されています。
...
coffee-scriptで.coffee上の行番号を出力させるにはcoffee-scriptのバージョンが __1.6.2以上__ である必要があります。
